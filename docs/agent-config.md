...
每个 host 仓库通过根目录的 `.hangrix/agents.yml` 声明 team 行为 —— 哪些 role、各自的 prompt、触发条件、`permission:` 仓库读写级别、`not:` 工具黑名单、容器环境、LLM 选型。**所有 agent 配置都在 host 仓库内部**：role 的提示词写成 inline `prompt:` 或 `.hangrix/prompts/<role>.md` 文件；版本固定 = host 仓库的 commit sha；没有第二个仓库或 lock 文件需要追踪。
...
roles:
  watcher:
    triggers:                     # 只读旁观者：监听评论但不改动任何东西
      issue.comment:
        mentioned_only: true
    permission: read              # 只能调只读端点；任何写操作服务端 403
    prompt_file: .hangrix/prompts/watcher.md
  ...
  backend:
    ...
    permission: write             # 可评论 / 创建 issue / 推贡献分支等
    not:                          # 但不可合并 —— 合并由 maintainer 负责
      - issue_merge
      - issue_close
    prompt: |
  ...
  reviewer:
    triggers:
      ...
      issue.comment:
        mentioned_only: true
    permission: write             # 投票（issue_review_vote）是写操作
    not: [issue_merge, issue_close]
  ...
  maintainer:
    triggers:
      ...
      issue.comment:
        mentioned_only: true
    permission: write             # 编排者：合并 / 关闭 / 发布，无黑名单
    prompt: |
...
- **`not:`** —— 工具黑名单（细粒度能力旋钮，叠加在 `permission` 之上）。列出的工具名（本地工具如 `bash`/`edit`，或平台工具如 `issue_merge`）会**直接从 agent 暴露给 LLM 的 tool schema 中隐藏**——模型根本看不到它们。**由 agent 侧强制（schema 隐藏），服务端不校验黑名单**：服务端只认 `permission`。所以 `not:` 用来塑形「这个角色该用哪些工具」，而 `permission` 才是安全边界。省略 = 不隐藏任何工具。
- **`mcp:`** —— 可选，MCP 服务器白名单。字符串数组，每项对应仓库根目录 `.mcp.json` 中 `mcpServers` 的一个 key。**缺失或空数组时该 role 不加载任何 MCP 服务器**；非空时仅加载列出的服务器。引用了 `.mcp.json` 中不存在的 server 名时 session 明确失败并报告缺失的 server 名。`mcp:` 是独立的 MCP 白名单，不复用 `not:` 的工具黑名单。
...
- **冻结点 = session spawn 那一刻。** 第一次唤醒某 role 时，按当时 host 仓库 base 分支的 commit 算 `repo_sha`，把解析后的 role 配置（prompt 内容 / `permission` 级别 / `not:` 工具黑名单 / resolved llm / container spec）一并 snapshot 进 session 元数据。**整 session 生命周期不再重读 host yaml** —— host yaml 中途改了不影响在跑的 session。同 issue 不同 role 各自冻结自己的 `repo_sha`；中途新加的 role 在它第一次被唤醒时拍自己的照。这个约束是 audit 可重现性的支点。
