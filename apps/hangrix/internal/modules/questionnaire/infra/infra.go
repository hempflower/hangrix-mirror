...
func (s *PostgresStore) InsertFirstAnswer(ctx context.Context, qID, actorID int64, perQ map[int64]domain.AnswerValue) (*domain.Answer, *domain.Questionnaire, error) {
	...
	a := answerFromRow(answerRow.ID, answerRow.QuestionnaireID, answerRow.ActorID, answerRow.Answers, answerRow.SubmittedAt, answerRow.UpdatedAt)
...
func (s *PostgresStore) GetUserAnswer(ctx context.Context, qID, actorID int64) (*domain.Answer, error) {
	row, err := s.q.GetActorAnswer(ctx, questionnairedb.GetActorAnswerParams{
		QuestionnaireID: qID,
		ActorID:         actorID,
	})
	if err != nil {
		return nil, fmt.Errorf("get actor answer: %w", err)
	}
	a := answerFromRow(row.ID, row.QuestionnaireID, row.ActorID, row.Answers, row.SubmittedAt, row.UpdatedAt)
	return &a, nil
}
...
func answerFromRow(id, qID int64, actorID int64, answers []byte, submittedAt, updatedAt pgtype.Timestamptz) domain.Answer {
	a := domain.Answer{
		ID:              id,
		QuestionnaireID: qID,
		ActorID:         actorID,
	...
	if len(answers) > 0 {
		a.PerQuestion = jsonToAnswerPerQuestion(answers)
	}
	return a
}
