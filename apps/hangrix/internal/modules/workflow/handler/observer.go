...
// and newSHA using git diff --name-only. Returns nil on any error — changed
// paths are a best-effort signal for workflow path filters.
func branchChangedPaths(ctx context.Context, fsPath, oldSHA, newSHA string) []string {
	...
	out, err := cmd.Output()
	if err != nil {
		log.Printf("workflow: PostReceive git diff --name-only %s..%s: %v", oldSHA, newSHA, err)
		return nil
	}
