...
const props = withDefaults(defineProps<{
  ...
  breaks?: boolean
  // Render-server-side passes already-trusted HTML; we still sanitize so a
  // compromised upstream can't slip script tags through.
}>(), {
  breaks: true,
})

// Parse synchronously — marked supports it as long as no async extensions
// are registered. Avoids a flash of unrendered text on initial paint.
const html = computed(() => {
