...
The host repo's agent roster lives at `.hangrix/agents.yml`. When you read, validate, or edit that file, the JSON Schema below is the runtime contract — the `internal/agentsconfig` parser enforces it. **Unknown keys are silently dropped** (forward-compat: a host may ship newer fields against an older agent server without bricking), but every other constraint — duplicate role keys, malformed value types, out-of-range numbers, mutual exclusions, the role-key / env-key / mount-path grammars — is still rejected. The footgun to remember: a typo'd field name does **not** error — it just stops doing anything. If a trigger fires more or less often than you expected, diff the keys you wrote against the canonical names below before assuming a runtime bug.
  ...
  "$defs": {
    "container": {
      ...
      "properties": {
        ...
        "env": {
          "type": "object",
          "description": "Plain-text env vars injected into every role container. Keys uppercase `[A-Z_][A-Z0-9_]*`.",
          "patternProperties": {
        ...
        "secrets": {
          "type": "array",
          "description": "Name-only list of secrets the platform injects from the repo's secrets settings. Values never appear in this file.",
          "items": {
            "type": "string",
    ...
    "triggerPushFilter": {
      "description": "Filter block for `commit.pushed`. AND-combined when both lists are set. Glob semantics mirror GitHub Actions / doublestar. Unknown keys are silently dropped — see `triggerCommentFilter` for the same forward-compat note.",
      "oneOf": [
        { "type": "null" },
        {
          "type": "object",
