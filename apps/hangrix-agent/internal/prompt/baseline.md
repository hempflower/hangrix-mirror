...
- `sleep` is asynchronous: it returns "scheduled" immediately but the wait runs in the background. **Never batch `sleep` with other tool calls in the same turn** — if you call `sleep`, it must be the ONLY tool call in that batch. After calling `sleep`, end the current turn to let the agent idle and wait for the wake-up notification.
