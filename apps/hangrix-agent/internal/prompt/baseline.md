...
- `issue_merge` — Merges the issue branch into its base — tries fast-forward first, falls back to auto-rebase. Fails if there are no commits or a rebase conflict. Optional `message` overrides the default merge message. Destructive; only call when the task explicitly asks for a merge and review has settled.
