...
```json
{
  ...
  "$defs": {
    ...
    "llm": {
      "type": "object",
      "required": ["model"],
      "description": "LLM tuning block. Appears at team level (top-level llm:) and per-role; role-level overrides team, team overrides platform default. The parser refuses an empty model — an llm block with no model is a misconfiguration, not 'use defaults'.",
      "properties": {
        "model": {
          "type": "string",
          "minLength": 1,
          "description": "Model name; must appear in some provider's allowed_models at runtime."
        },
        "reasoning_effort": {
          "description": "Thinking intensity. OpenAI-compat adapters pass it through; the Anthropic adapter translates to thinking.budget_tokens (minimal/low → 1024, medium → 4096, high → 16384) and drops temperature. Empty string is equivalent to omission.",
